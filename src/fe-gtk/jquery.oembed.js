...
(function($) {
    ...
    $.fn.oembed.insertCode = function(container, embedMethod, oembedData) {
		//XXX: HACKONE DIARREA DURA
		tmpnode=$(oembedData.code)[0]
		srcc=$(tmpnode).attr('src');
		if(srcc){$(tmpnode).attr('src',srcc+'?frame=clarochesi'); oembedData.code=$(tmpnode)[0].outerHTML;}
        ...
        switch (embedMethod) {
          ...
          case "append":
			  ...
			  if (settings.includeHandle) {
				  $('<span class="oembedall-closehide"><i class="icon-circle-arrow-up"></i></span>').insertBefore(container).click(function() {
					  var icoval = $("i",this).attr("class");
					  $(this).html((icoval == 'icon-circle-arrow-up') ? '<i class="icon-circle-arrow-down"></i>' : '<i class="icon-circle-arrow-up"></i>');
    ...
    new $.fn.oembed.OEmbedProvider("wikipedia", "rich", ["wikipedia.org/wiki/.+"], "http://$1.wikipedia.org/w/api.php?action=parse&page=$2&format=json&section=0&callback=?",{
      ...
      templateData : function(data){if(!data.parse)return false;
          ...
          return  '<div id="content"><h3 class="wikititle"><a class="nav-link" href="http://en.wikipedia.org/wiki/'+data.parse['displaytitle']+'">'+data.parse['displaytitle']+'</a></h3><div class="wikitext">'+text+'</div></div>';
        }
      }),
